...
scores_normal_path <- "./data/statistics/03_statistics_control_scores_2025-05-20.csv"
...
# Step 2: Rename .y columns to remove the .y suffix
...
data <- data %>%
  ...
  arrange(cancer_status, sorting_value, cancer_type, source, group_id)
  # arrange(source, sorting_value, cancer_type, group_id)
...
HBOC_breast_cutoff <- 0.0396
HBOC_ovarian_cutoff <- 0.18
```
...
data$HBOC_breast <- ifelse(data$HBOC_breast >= HBOC_breast_cutoff, "pos", "neg")
data$HBOC_breast[is.na(data$`cfMeDIP`) | data$`cfMeDIP` == ""] <- "NS"

data$HBOC_ovarian <- ifelse(data$HBOC_ovarian >= HBOC_ovarian_cutoff, "pos", "neg")
data$HBOC_ovarian[is.na(data$`cfMeDIP`) | data$`cfMeDIP` == ""] <- "NS"
...
```{r}
# only select samples where integrated_ctDNA_score is positive 
# First filter `data` as before
# data <- data[data$integrated_ctDNA_score == "pos" & !is.na(data$integrated_ctDNA_score), ]
# 
# # Then filter `data_bar` to only matching group_id values
# data_bar <- data_bar[data_bar$group_id %in% data$group_id, ]

# Only select samples where integrated_ctDNA_score is positive and HBOC scores are not NS
data <- data[
  data$integrated_ctDNA_score == "pos" &
  !is.na(data$integrated_ctDNA_score) &
  data$HBOC_breast != "NS" &
  data$HBOC_ovarian != "NS",
]
...
```{r}
data_mutation <- as.matrix(data[, c("Mutation", "Tumour.Fraction","FS", "consensus_griffin", "fragment_ratio_corr", "avg_zscore_nuc_peaks", "integrated_ctDNA_score", "HBOC_breast", "HBOC_ovarian")])

colnames(data_mutation) <- c("Somatic Mutation", "CNA Tumour Fraction", "Fragment Length Score", "Griffin Cancers Scored Positive", "Fragment Ratio", "Nucleosome Peak Score", "Integrated Score", "HBOC breast", "HBOC ovarian")
...
# Replace "NS" and empty values with NA
# data_positive[data_positive == "NS" | data_positive == ""] <- NA
# data_negative[data_negative == "NS" | data_negative == ""] <- NA
...
col_type <- c(Ovarian = "#fd7f6f",  Breast = "#bd7ebe", Cervical = "#7eb0d5", Lung="#8bd3c7", Mucinous= "#b2e061",Neuroendocrine="#ffb55a", "Peritoneal Papillary Serous Carcinoma" ="#ebdc78", Prostate="#beb9db", Liver="#fdcce5", "NA" = "white", Endocrine = "#6a0dad", Thyroid = "#ff6347" , Thymoma="#4682b4", Myeloma="#9acd32", Endometrial = "blue")
...
# Define the right annotation without borders around bars and without titles for barplots
right_annotation <- rowAnnotation(
  # Display percentage text for Positive samples
  Positive_Percent = anno_text(percent_positive, gp = gpar(fontsize = 12)),
  
  # Create barplot for Positive samples, no border
  Positive_Barplot = anno_barplot(
    positive_counts,  # Sum of 1's across each mutation type
    border = FALSE,
    width = unit(2.8, "cm"),
    gp = gpar(fill = "#fb9a99", col = NA),  # Color for positive barplot
    axis_param = list(side = "top", labels_rot = 0, gp = gpar(fontsize = 10))
  ),
  
  # Display percentage text for Negative samples
  Negative_Percent = anno_text(percent_negative, gp = gpar(fontsize = 12)),
  
  # Create barplot for Negative samples, no border
  Negative_Barplot = anno_barplot(
    negative_counts,  # Sum of 1's across each mutation type
    border = FALSE,
    width = unit(2.8, "cm"),
    gp = gpar(fill = "#fb9a99", col = NA),  # Color for negative barplot
    axis_param = list(side = "top", labels_rot = 0, gp = gpar(fontsize = 10))
  ),
  
  # Hide annotation names (titles) for each component
...
legend_months <- Legend(
  ...
  at = c("<3 months", "3 - 6 months", "6-12 months", "12-24 months", "24-36 months", 
         "no future cancer", "unknown", "positive", "pending"),
  legend_gp = gpar(fill = c("black", "darkgrey", "grey", "lightgrey", "white", 
                            "green", "yellow", "black", "blue")),  # Positive is also white
...
annotation_legend <- packLegend(
  list = list(
    Legend(title = "Cancer Type",
           at = c("Ovarian", "Cervical", "Breast", "Lung", "Mucinous", "Neuroendocrine", 
                  "Peritoneal Papillary Serous Carcinoma", "Prostate", "Liver", "NA", 
                  "Endocrine", "Thyroid", "Thymoma", "Myeloma", "Endometrial"),
           legend_gp = gpar(fill = c("#fd7f6f","#7eb0d5","#bd7ebe","#8bd3c7", "#b2e061",
                                     "#ffb55a","#ebdc78","#beb9db", "#fdcce5", "white",
                                     "#6a0dad", "#ff6347" ,"#4682b4","#9acd32", "blue")),
...
top_annotation <- HeatmapAnnotation(
  ...
  "HBOC Breast" = anno_barplot(
    ifelse(is.na(data_bar$HBOC_breast), 0, data_bar$HBOC_breast),
    gp = gpar(
      fill = ifelse(data_bar$HBOC_breast > HBOC_breast_cutoff, "red", "black"),
      col = ifelse(data_bar$HBOC_breast > HBOC_breast_cutoff, "red", "black")
    ),
    bar_width = 0.7,
    axis_param = list(gp = gpar(fontsize = 8)),
    border = FALSE
  ),
  "HBOC Ovarian" = anno_barplot(
    ifelse(is.na(data_bar$HBOC_ovarian), 0, data_bar$HBOC_ovarian),
    gp = gpar(
      fill = ifelse(data_bar$HBOC_ovarian > HBOC_ovarian_cutoff, "red", "black"),
      col = ifelse(data_bar$HBOC_ovarian > HBOC_ovarian_cutoff, "red", "black")
    ),
    bar_width = 0.7,
    axis_param = list(gp = gpar(fontsize = 8)),
    border = FALSE
...
```{r}
## Generate heatmap
pdf(file.path(outdir, "03_figure_cfMeDIP_integrated_barplot_by_cancer_status_with_methylation_integration_positive.pdf"), width = 15, height =10)
oncoPrint <- oncoPrint(data_mutation,
                       ...
                       height = unit(0.44*nrow(data_mutation), "cm"),
                       width = unit(0.35*ncol(data_mutation), "cm"))
...
decorate_annotation("Positive_Barplot", {
  grid.text("% Positive in \n HBOC Positive", 
            x = unit(.35, "npc"), y = unit(1.2, "npc") + unit(6, "mm"), # Adjust y position if needed
            gp = gpar(fontsize = 12))
})

decorate_annotation("Negative_Barplot", {
  grid.text("% Positive in \n HBOC Negative", 
            x = unit(.4, "npc"), y = unit(1.2, "npc") + unit(6, "mm"),
            gp = gpar(fontsize = 12))
})
...
decorate_annotation("CNA Tumour\nFraction", {
  grid.lines(
    c(0.5, 43.5),  # x-coordinates: from the left edge to the right edge
...
decorate_annotation("Griffin Cancers\nScored Positive", {
  grid.lines(
    c(0.5, 43.5),  # x-coordinates: from the left edge to the right edge
...
decorate_annotation("HBOC Breast", {
  grid.lines(
    c(0.5, 43.5),  # x-coordinates: from the left edge to the right edge
    c(HBOC_breast_cutoff, HBOC_breast_cutoff),  # y-coordinate for the line
    gp = gpar(lty = 2, col = "red", lwd = 2.2),  # Dashed red line
    default.units = "native"
  )
})

decorate_annotation("HBOC Ovarian", {
  grid.lines(
    c(0.5, 43.5),  # x-coordinates: from the left edge to the right edge
    c(HBOC_ovarian_cutoff, HBOC_ovarian_cutoff),  # y-coordinate for the line
    gp = gpar(lty = 2, col = "red", lwd = 2.2),  # Dashed red line
    default.units = "native"
  )
})
