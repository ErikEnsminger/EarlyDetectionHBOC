---
title: "01_proessing_cohort_breakdown"
...
samples <- samples[, c("group_id", "LIB_ID", "timepoint", "Library Name", "TS", "date_of_blood", "age_at_blood", "cancer_status", "source", "gene", "sex")]
head(samples)
```
...
male_samples <- samples %>% filter(sex == "male") %>% nrow()

n_females <- sum(sex_per_patient$sex == "female")
female_samples <- samples %>% filter(sex == "female") %>% nrow()
...
category_summary <- samples %>%
  group_by(source) %>%
  summarise(individuals = n_distinct(LIB_ID), samples = n()) %>%
...
# Summary of how many had 1, 2, 3... samples
...
n_both <- sum(samples$has_WGS & samples$has_TS)
n_wgs_only <- sum(samples$has_WGS & !samples$has_TS)
n_ts_only <- sum(samples$has_TS & !samples$has_WGS)
...
Out of all samples, {n_both} were submitted for a combined WGS and TS protocol, {n_wgs_only} were submitted only for WGS, and {n_ts_only} were submitted only for TS sequencing.
"))
```
...
time_summary <- samples %>%
  filter(!is.na(date_of_blood)) %>%
  group_by(LIB_ID) %>%
  filter(n_distinct(group_id) >= 2) %>%
  summarise(
