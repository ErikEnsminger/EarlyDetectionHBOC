...
metadata <- metadata %>%
  ...
  dplyr::mutate(
    ...
    time_from_baseline = as.numeric(difftime(date_of_blood, baseline, units = "days")) / 30.44
  ) %>%
  dplyr::ungroup()
```
...
data_sero <- metadata %>%
  dplyr::group_by(LIB_ID) %>%
  dplyr::arrange(as.numeric(timepoint)) %>%  # ensure correct order
  dplyr::summarise(
    converter = dplyr::case_when(
      # Active Cancer: positive throughout or single positive timepoint
      all(cancer_status == "positive") ~ "Active Cancer",
      ...
      # Forward Phenoconverter: starts negative and has at least one positive later
      n() > 1 &
        dplyr::first(cancer_status) == "negative" &
        any(cancer_status == "positive") ~ "Forward \nPhenoconverters",
      
      # Reverse Phenoconverter: starts positive and has at least one negative later
      n() > 1 &
        dplyr::first(cancer_status) == "positive" &
        any(cancer_status == "negative") ~ "Reverse \nPhenoconverters",
      
      # Multiple Cancers: everything else (mixed / weird patterns)
      TRUE ~ "Multiple Cancers"
    ),
    .groups = "drop"
  )

data_sero$converter <- factor(
  data_sero$converter,
  levels = c(
    "Multiple Cancers",
    "Forward \nPhenoconverters",
