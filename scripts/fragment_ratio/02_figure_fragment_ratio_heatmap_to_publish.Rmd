...
scores_path <- here::here("raw_data", "ichorCNA", "2025-02-28_ichorCNA_HBOC_Feb28_2025_ichorCNA_estimates.tsv")
scores <- read.delim(scores_path, sep = "\t")[, c("ID", "Tumour.Fraction")]

scores_normal_path <- here::here("raw_data", "ichorCNA", "2025-02-28_ichorCNA_Healthy_Control_V3_ichorCNA_estimates.tsv")
...
right_annotation <- rowAnnotation(
  # 1) Mutation annotations
  BRCA1 = anno_simple(data_corr_df$BRCA1_somatic, col = col, simple_anno_size = unit(1.5, "mm")),
  BRCA2 = anno_simple(data_corr_df$BRCA2_somatic, col = col, simple_anno_size = unit(1.5, "mm")),
  TP53  = anno_simple(data_corr_df$TP53,  col = col, simple_anno_size = unit(1.5, "mm")),
  PALB2 = anno_simple(data_corr_df$PALB2, col = col, simple_anno_size = unit(1.5, "mm")),
  ...
  "Tumour\nFraction" = anno_lines(
    data_corr_df[, c(
      "TumourFraction","LOD_TF",
      "vaf_BRCA1_somatic","vaf_BRCA2_somatic","vaf_TP53","vaf_PALB2"
    )],
    add_points = TRUE,
    pch   = c(16, NA, 4, 4, 4, 4),
    # use "transparent" for no‐line color, and "blank" for no‐line type
    gp    = gpar(
      col = c(NA, "red", rep("transparent", 4)),
      lty = c("solid","dashed", rep("blank",     4))
    ),
    pt_gp = gpar(
      col = c("black",   NA,   rep("red",       4))
    ),
    size       = unit(1, "mm"),
    axis_param = list(
      side       = "bottom",
  ...
  "  " = anno_empty(which = "row", border = FALSE, width = unit(2, "mm")),  # second spacer
  ...
  annotation_name_gp   = gpar(fontsize = 10.5),
  border               = FALSE
)
```
