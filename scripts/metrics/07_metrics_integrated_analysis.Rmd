...
#   library(data.table)
# 
#   # Step 1: Define future cancer samples
#   months_numeric <- suppressWarnings(as.numeric(data$months_to_positive))
#   future_cancer_samples <- data[!is.na(months_numeric), ]
#   
#   if (!is.null(months_limit)) {
#     future_cancer_samples <- future_cancer_samples[months_numeric[!is.na(months_numeric)] <= months_limit, ]
#   }
#   
#   # Step 2: Total future cancer samples & patients
#   n_future_cancer <- nrow(future_cancer_samples)
#   n_future_with_signal <- sum(future_cancer_samples$n_signals >= signal_limit)
#   
#   percent_future_signal <- ifelse(n_future_cancer > 0, round(n_future_with_signal / n_future_cancer * 100, 1), NA)
#   
...
#   percent_patients_with_signal <- ifelse(n_future_patients > 0, round(n_future_patients_with_signal / n_future_patients * 100, 1), NA)
#   
#   # NEW: Max/median/mean months_to_positive among signal-positive future cancer patients
#   future_with_signal <- future_cancer_samples[future_cancer_samples$LIB_ID %in% future_cancer_ids_with_signal, ]
#   future_with_signal$months_numeric <- suppressWarnings(as.numeric(future_with_signal$months_to_positive))
#   dt <- as.data.table(future_with_signal)
#   max_months_per_patient <- dt[, .(max_month = max(months_numeric, na.rm = TRUE)), by = LIB_ID]
#   
#   max_months_signal_pos <- ifelse(nrow(max_months_per_patient) > 0, max(max_months_per_patient$max_month, na.rm = TRUE), NA)
#   median_months_signal_pos <- ifelse(nrow(max_months_per_patient) > 0, median(max_months_per_patient$max_month, na.rm = TRUE), NA)
#   mean_months_signal_pos <- ifelse(nrow(max_months_per_patient) > 0, round(mean(max_months_per_patient$max_month, na.rm = TRUE), 1), NA)
# 
#   # Step 3: Signal-positive overview among the cancer-negative group
#   n_signal_samples <- sum(data$n_signals >= signal_limit)
#   n_signal_patients <- length(unique(data$LIB_ID[data$n_signals >= signal_limit]))
#   
#   percent_signal_samples_future_cancer <- ifelse(n_signal_samples > 0, round(n_future_with_signal / n_signal_samples * 100, 1), NA)
#   percent_signal_patients_future_cancer <- ifelse(n_signal_patients > 0, round(n_future_patients_with_signal / n_signal_patients * 100, 1), NA)
#   
#   # Step 4: Output
#   if (!is.null(months_limit)) {
...
#   
# 
#   cat("\n===== Proportion relative to all Signal-Positive Samples / Patients =====\n")
#   cat("Signal-positive samples (in full cancer-negative cohort):", n_signal_samples, "\n")
#   cat("→ Of those, develop cancer", ifelse(!is.null(months_limit), paste0(" within ", months_limit, " months:"), " in future:"), 
...
future_cancer_summary(cancer_negative, group_label="(Cancer-Negative Samples)", months_limit=3)
```
...
```{r}
# Future cancer within 6 months
future_cancer_summary(cancer_negative, group_label="(Cancer-Negative Samples)", months_limit=24)
```
