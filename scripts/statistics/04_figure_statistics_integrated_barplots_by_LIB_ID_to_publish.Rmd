---
title: "04_figure_statistics_integrated_barplots_by_LIB_ID_to_publish"
...
# sizing parameters
# ──────────────────────────────────────────────
cell_h_cm            <- 1.2   # your heatmap row / anno height
cell_w_cm            <- 0.35  # your heatmap column / barplot width
...
```{r}
samples <- samples[, c("group_id","Library Name", "TS", "source", "cancer_type", "survivor", "date_of_blood", "months_to_positive")]
```
...
mut_map <- c(
  ...
  "Frame_Shift_Del;Frame_Shift_Del;Frame_Shift_Del"  = "frameshift;frameshift;frameshift",
  "Missense_Mutation;Missense_Mutation"             = "missense;missense",
  "Missense_Mutation;Frame_Shift_Del"               = "missense;frameshift",
  "Nonsense_Mutation;Missense_Mutation"             = "stop;missense"
)

# 2) apply it only to character columns
data <- data %>%
  mutate(across(
    .cols = where(is.character),
    .fns  = ~ str_replace_all(.x, mut_map)
  ))
```
...
data <- data %>%
  mutate(
    sorting_value = case_when(
      ...
      months_to_positive == "pending" ~ Inf,
...
```{r}
outpath_path_2 <- paste0(outdir_data, "/04_figures_statistics_numerical_scores.csv")
write.csv(data, outpath_path_2, row.names = FALSE)
```

```{r}
outpath_path <- paste0(outdir_data, "/04_figures_statistics_data.csv")
...
annotation_legend <- packLegend(
  list = list(
    Legend(title = "Cancer Type",
           ...
           title_gp = gpar(fontsize = 30),     # <- larger title
           labels_gp = gpar(fontsize = 25),    # <- larger labels
           grid_height = unit(4, "mm"),        # <- larger colored boxes
           nrow = 4),
...
```{r}
## Generate heatmap
pdf(file.path(outdir, "04_figure_statistics_integrated_barplot_by_LIB_ID_to_publish.pdf"), width = 29, height =14)
oncoPrint <- oncoPrint(
  data_mutation,
  # row_split = rownames(data_mutation), 
  alter_fun         = alter_fun,
  col               = col,
  column_split      = col_split,
  column_gap          = unit(0.2, "cm"),
  # hide the split‐group labels:
  column_title      = NULL,
