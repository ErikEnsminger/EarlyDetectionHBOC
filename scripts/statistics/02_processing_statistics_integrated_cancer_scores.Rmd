...
# data$consensus_griffin <- scale(log1p(data$consensus_griffin))
# data$Tumour.Fraction <- scale(log1p(data$Tumour.Fraction))
# data$FS <- scale(log1p(data$FS))

# data$consensus_griffin <- log1p(data$consensus_griffin)
data$Tumour.Fraction <- log1p(data$Tumour.Fraction)
data$FS <- log1p(data$FS)
data$fragment_ratio_corr <- -log1p(1 - data$fragment_ratio_corr)


filtered_data <- data

# Exclude Specific Samples
samples_to_exclude <- c("LIB-04-0376-T2", 
                        "LIB-04-0428-T3", 
                        "LIB-04-0154-T0", 
                        "LIB-04-0664-T0")

# Convert Mutation to binary and exclude specific samples
filtered_data <- filtered_data %>%
  dplyr::mutate(Mutation_binary = ifelse(
    group_id %in% samples_to_exclude, NA, 
    ifelse(is.na(Mutation) | Mutation == "", 0, 1)
  )) %>%
  dplyr::filter(!is.na(Mutation_binary))  # Remove excluded samples
...
filtered_data <- filtered_data[,c("group_id", "roc_label","FS",  "avg_zscore_nuc_peaks", "fragment_ratio_corr", "Tumour.Fraction", "consensus_griffin", "Mutation_binary")]
...
roc_columns <- c(
  ...
  "Mutation_binary" = "Mutation Status"
)
```
...
train_ctDNA_model <- function(data, outcome_col = "roc_label", 
                              features = c("FS", "avg_zscore_nuc_peaks", "fragment_ratio_corr", 
                                           "Tumour.Fraction", "consensus_griffin", "Mutation_binary"), 
